    ...
    return (
      <motion.div ref={scope} className="inline">
        {wordsArray.map((word, idx) => {
          return (
            <div key={`word-${idx}`} className="inline-block text-left">
...
export const TypewriterEffectSmooth = ({
          ...
          return (
            <div key={`word-${idx}`} className="inline-block">
              {word.text.map((char, index) => (
                <span
                  key={`char-${index}`}
                  className={cn(` dark:text-sky-50 `, word.className)}
                >
                  {char}
                </span>
              ))}
  ...
  return (
    <div className={cn("my-6 flex", className)}>
      <motion.div
      ...
      <motion.span
        ...
        className={cn(
          "block h-4 w-[2px]  rounded-sm bg-blue-500 sm:h-6 xl:h-12",
          cursorClassName,
        )}
      ></motion.span>
    </div>
